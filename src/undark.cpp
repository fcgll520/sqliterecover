...
int tdump( char *p, uint16_t l ) {
	...
	while (l--) {
		if (*p >= 0x20 && *p <= 0x7e/*isprint(*p)*/) fprintf(stdout, "%c", *p); else fprintf(stdout, ".");
		p++;
	}
...
\------------------------------------------------------------------*/
int WStringToString(const std::wstring &wstr, std::string &str)
{
	int dwMinSize = WideCharToMultiByte(CP_ACP, 0, (LPCWSTR)wstr.c_str(), -1, NULL, 0, NULL, FALSE);
	//char *arcRealPath = (char*)malloc(dwMinSize);
	str.resize(dwMinSize, '\0');

	int nResult = WideCharToMultiByte(CP_ACP, 0, (LPCWSTR)wstr.c_str(), -1, (LPSTR)str.c_str(), dwMinSize, NULL, NULL);
...
int sqltdump( char *p, uint16_t l ) {
	...
	//UTF8
	boost::scoped_array<char> pBuffer(new char[l + 1]);
	*(pBuffer.get() + l) = 0;
	memcpy(pBuffer.get(), p, l);
	std::wstring strValue = boost::locale::conv::utf_to_utf<wchar_t>(pBuffer.get());
	std::string strValueNew;
	WStringToString(strValue, strValueNew);
	printf("%s", strValueNew.c_str());
	fprintf(stdout,"\"");
...
int hdump( unsigned char *p, uint16_t length, char *msg ) {
	...
	while (ll > 0) {
		...
		while (ll--) {
			fprintf(stdout, "%c", (*p >= 0x20 && *p <= 0x7e)/*isprint(*p)*/ ? *p : '.');
			c++;
			p++;
			if (c%16 == 0)  break;
		}
...
int blob_dump_to_file( struct globals *g, char *p, size_t l ) {
	...
	_snprintf(fn, sizeof(fn), "%d.blob", g->blob_count);
...
int decode_row( struct globals *g, char *p, char *data_endpoint, struct sql_payload *payload, int mode, size_t forced_length ) {
	...
	if (payload->length > (g->page_size -35)) {
		...
		endian_swap(tmp);
		ovp = payload->overflow_pages[0] = tmp;
		...
		while (ovp > 0) {
			...
			endian_swap(tmp);
			ovp = payload->overflow_pages[ovpi] = tmp;
...
int dump_row( struct globals *g, char *base, char *data_endpoint, struct sql_payload *payload, int mode ) {
	...
	if (payload->overflow_pages[0] == 0) {
		...
		payload->mapped_data = (char*)malloc( (payload->length +100) *sizeof(char) );
		...
		while (payload->overflow_pages[ovpi]) {
			DEBUG fprintf(stdout,"Copying data from file to memory for page %d to offset [%d]\n", payload->overflow_pages[ovpi], (int)(payload->mapped_data_endpoint -payload->mapped_data));
				...
				if (( addr < (void *)g->db_origin) || ( (char*)addr+4 > (void *)g->db_end)) {
		...
		if (t>=0) { fprintf(stdout,",");
			switch (payload->cells[t].t) {
				...
				case 2: {
							  ...
							  endian_swap(n);
							  fprintf(stdout, "%d", to_signed_int(n));
						  }
						  break;
				...
				case 3: {
							  ...
							  endian_swap(n);
							  fprintf(stdout,"%ld", to_signed_long(n));
						  }
						  break;
				...
				case 5:
				{
					int tmp = *(payload->mapped_data + payload->cells[t].o);
					endian_swap(tmp);
...
int main( int argc, char **argv ) {
	 ...
	 */
	fd = fopen( g->input_file, "rb" );
	g->db_size = st.st_size;
	char *fbuffer = (char*)malloc(g->db_size + 1);
	memset(fbuffer, 0, g->db_size + 1);
	fread(fbuffer, g->db_size, 1, fd);
	g->db_origin = fbuffer;//mmap( NULL, st.st_size, PROT_READ, MAP_PRIVATE, fd, 0 );
	...
	endian_swap(g->page_count);
	//g->page_count = atol( g->page_count ); // convert to local format
	...
	//g->freelist_first_page = atol( g->freelist_first_page );
	endian_swap(g->freelist_first_page);
	...
	//g->freelist_page_count = atol( g->freelist_page_count );
	endian_swap(g->freelist_page_count);
	...
	if (0) {
		if (g->freelist_page_count) {
			g->freelist_pages = (uint32_t*)malloc((g->freelist_page_count + 1) *sizeof(uint32_t));
			if (!g->freelist_pages) {
				...
				if ( pli < g->freelist_page_count ) {
					do {
						...
						//tmp_page = atol(tmp_page);
						endian_swap(tmp_page);
						fp += sizeof(uint32_t);
						DEBUG fprintf(stdout,"Next trunk page (if any): %d\n",tmp_page);
						DEBUG fflush(stdout);

						memcpy( &leaf_page_count, fp, sizeof(uint32_t));
						//leaf_page_count = atol(leaf_page_count);
						endian_swap(leaf_page_count);
						fp += sizeof(uint32_t);
						...
						while (( fp < current_page_endpoint )&&( leaf_page_count-- )) {
							...
							//g->freelist_pages[pli] = atol( g->freelist_pages[pli] );
							endian_swap(g->freelist_pages[pli]);
		...
		while (g->db_cfp < g->db_end ) {
			...
			DEBUG {
				...
				l = g->page_size;
				while ((l--)&&(p)) {
					{ if (*p >= 0x20 && *p <= 0x7e/*isprint(*p)*/) { fprintf(stdout, "%c", *p); } else fprintf(stdout, "_"); }
					p++;
					bc++;
			...
			if (*(g->db_cfp) == 13) { 
				...
				//leaf.freeblock_offset = atoi(leaf.freeblock_offset);
				endian_swap(leaf.freeblock_offset);
				if (leaf.freeblock_offset > 0) {
					...
					//leaf.freeblock_next = atoi(leaf.freeblock_next);
					endian_swap(leaf.freeblock_next);
					memcpy( &(leaf.freeblock_size), ( g->db_cfp +leaf.freeblock_offset +2 ), 2 );
					//leaf.freeblock_size = atoi(leaf.freeblock_size);
					endian_swap(leaf.freeblock_size);
				}
				...
				fflush(stdout);
				//				leaf.freeblock_offset = atoi( ta );
				{
					short tmp = *(g->db_cfp + 3);
					endian_swap(tmp);
					leaf.cellcount = tmp;
				}
				{
					short tmp = *(g->db_cfp + 5);
					endian_swap(tmp);
					leaf.cell_offset = tmp;
				}
